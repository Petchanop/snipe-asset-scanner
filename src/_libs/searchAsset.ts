import { AddAssetCountLine } from "@/_repositories/assetCountLine"
import { AssetResponse } from "@/_intergrations/snipeit/snipe-it"
import { assetStatusOptions } from "@/_constants/constants"
import { AssetCount, AssetCountLine, 
    TAssetRow, AssetCountLocation, 
    User, assetUser, ExtendAssetResponse } from "@/_types/types"
...
export async function CreatAssetCountLine(
    ...
    const extendTypeAsset: ExtendAssetResponse = {
        ...
        in_report: assetInReport.find(
            (report) => report.asset_code === data.asset_tag
        ) ? true : false,
        location_id: locationId.id,
        is_assigned_incorrectly: false,
        status: assetStatusOptions
            .find((status) => 
                status.value.toLowerCase() == data.status_label?.status_meta as string
        )?.value as string,
...
export async function UpdateAssetCountLineForSearchAssetPage(
    ...
    if (!IsInLocation) {
        ...
        const result = await UpdateAssetCountLine(IsInLocation.id, 
            { asset_check: true, checked_by: parseInt(user.id) as number }
        )
        const assigned_to = users.find(
            (data: User) => data.id == result?.assigned_to as number
        )
        asset = {
            id: result?.id,
